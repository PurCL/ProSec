...
def prepare_selection_dataset_new(
        ...
        for p_l_c, impls in tqdm(ori_prompt_lang_cwe2impl.items(), desc="Filtering out unparsable implementations"):
            ...
            for impl in impls:
                ...
                for code_block in impl['code_blocks']:
                    ...
                    #     empty_block_count += 1
                    # try:
                    #     parsed_code = parser.parse(bytes(code_block, "utf8"))
                    # except TypeError:
                    #     print('typeerror:', code_block)
                    #     continue
                    # if not parsed_code.root_node.has_error:
                    #     parsable_impls.append(code_block)
    ...
    if lang_cwe_distinct:
        ...
        for entry in tqdm(fix_pair_ds, desc="Generating additional entries"):
            ...
            # if inducing_prompt not in inducing2impl_cnt:
            #     inducing2impl_cnt[inducing_prompt] = 0
            # inducing2impl_cnt[inducing_prompt] += len(parsable_impls)
            # if inducing_prompt not in inducing2ori_cnt:
            #     inducing2ori_cnt[inducing_prompt] = 0
            # inducing2ori_cnt[inducing_prompt] += 1
        # plot histogram of parsable impls
        # import matplotlib.pyplot as plt
        # plt.hist([len(ori_prompt2parsable_impl[prompt]['impls']) for prompt in ori_prompt2parsable_impl])
        # plt.savefig(os.path.join(cache_dir, 'histogram.png'))
        # plt.figure()
        # plt.hist([inducing2impl_cnt[k] for k in inducing2impl_cnt])
        # plt.savefig(os.path.join(cache_dir, 'histogram2.png'))
        # plt.figure()
        # plt.hist([inducing2ori_cnt[k] for k in inducing2ori_cnt])
        # plt.savefig(os.path.join(cache_dir, 'histogram3.png'))
        
        # downsample additional entries to 35% to speed up
        ...
        for entry in tqdm(fix_pair_ds, desc="Generating additional entries"):
            ...
            # downsample impls to 30% to speed up (NOTE: not sure if sampling here is optimal)
            torch.random.manual_seed(42)
...
if __name__ == '__main__':
    ...
    # prepare_selection_dataset(language='python', cached=False)
    # prepare_selection_dataset(language=None, cached=False)

    # phi3m
    # tmp_ds = prepare_selection_dataset_new(
    #     inst_dataset_name_or_path='PurCL/secalign-all-haiku-inst-clustered2k',
    #     dsec_name_or_path='prosecalign/secalign-haiku-new-fix-new-dsec-phi3m-all-pairs',
    #     # dsec_name_or_path='PurCL/secalign-claude-phi3mini-inst-all-pair',   # old
    #     dnorm_json_path='<your-path>/infer-ret-ori-task-haiku.phi3m-inst-filtered.jsonl',
    #     # dnorm_json_path='../infer-ret-ori-task-haiku.phi3m-inst.jsonl',  # old
    #     language=None, 
    #     cached=False,
    #     # cached=True,
    #     cache_dir='.cache/phi3m',
    #     lang_cwe_distinct=True
    # )
    # tmp_ds.save_to_disk('selection_ds_phi3m_filtered')
    # print(tmp_ds)
    
    # codellama
    # tmp_ds = prepare_selection_dataset_new(
    #     inst_dataset_name_or_path='PurCL/secalign-all-haiku-inst-clustered2k',
    #     dsec_name_or_path='prosecalign/secalign-haiku-new-fix-new-dsec-clm7b-all-pairs',
    #     dnorm_json_path='<your-path>/infer-ret-all.clm7b-inst-filtered.jsonl',
    #     # dnorm_json_path='<your-path>/infer-ret-all.clm7b-inst.jsonl',
    #     language=None, 
    #     # cached=False,
    #     cached=True,
    #     cache_dir='.cache/clm7b',
    #     lang_cwe_distinct=True
    # )
    # tmp_ds.save_to_disk('selection_ds_clm7b_filtered')
    # print(tmp_ds)

    # phi3m-0212
    # tmp_ds = prepare_selection_dataset_new(
    #     inst_dataset_name_or_path='PurCL/secalign-all-haiku-inst-clustered2k',
    #     dsec_name_or_path='prosecalign/secalign-haiku-phi3m-codebleu-dsec-all-pairs',
    #     dnorm_json_path='<your-path>/infer-ret-ori-task-haiku.phi3m-inst-filtered.jsonl',
    #     language=None,
    #     # cached=False,
    #     cached=True,
    #     cache_dir='.cache/phi3m-0212',
    #     lang_cwe_distinct=True
    # )
    # tmp_ds.save_to_disk('selection_ds_phi3m-0212')
    # print(tmp_ds)

    # codellama-0219
    tmp_ds = prepare_selection_dataset_new(
        inst_dataset_name_or_path='PurCL/secalign-all-haiku-inst-clustered2k',
        dsec_name_or_path='prosecalign/secalign-haiku-codebleu-clm7b-all-pairs',
        dnorm_json_path='<your-path>/infer-ret-all.clm7b-inst-filtered.jsonl',
        # dnorm_json_path='<your-path>/infer-ret-all.clm7b-inst.jsonl',
        language=None, 
        cached=False,
        # cached=True,
        cache_dir='.cache/clm7b-0219',
        lang_cwe_distinct=True
    )
    tmp_ds.save_to_disk('selection_ds_clm7b-0219')
    print(tmp_ds)