...
MODEL_NAME_SHORT=llama3.2-1b-inst
OUT_DIR=infer-ret-claude-haiku-inst-$MODEL_NAME_SHORT
...
function infer_one() {
    ...
    local_fout="--fout $OUT_DIR/infer-ret-$2-$MODEL_NAME_SHORT.jsonl"
    local_model="--model_name_or_path $MODEL_NAME"
    # python3 src/gen_inferences.py $local_ds --n 10 $local_fout $local_model --col_name original_prompt
    python3 src/gen_inferences.py $local_ds --n 20 $local_fout $local_model --col_name task
}
